    ...
    linux_luna_pkg = {
      ...
      buildLinux (args
        // rec {
          ...
          nativeBuildInputs = with pkgs; [llvmPackages_18.lld llvmPackages_18.clangUseLLVM pkgs.llvmPackages_18.llvm];
          makeFlags = [
            "LLVM=1"
            "LLVM_IAS=1"
          ...
          extraStructuredConfig = with lib.kernel; {
            ...
            # disable a ton of sound stuff i don't need
            ...
            #disiable all wiif except ilwifi
            ...
            # I don't need xen disable it
            XEN = lib.mkForce lib.kernel.no;
            # obscure af fs and also fuck Reiserfs
        ...
        // (args.argsOverride
          or {}));
    linux_luna = pkgs.callPackage linux_luna_pkg {};
  in
    pkgs.recurseIntoAttrs (pkgs.linuxPackagesFor linux_luna);
}
